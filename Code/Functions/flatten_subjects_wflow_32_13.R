#this function takes the subject_data column from our zooniverse data (the column that includes the information from the manifest file) and flattens it into a separate column for each json element.It is customized to match the json formatting from the "Species idenfitication" workflow (#16695) workflow version 32.13

#The only argument passed to it is the data frame containing the subject_data column.

flatten_subjects_wflow_32_13<-function(DF){
...
require(jsonlite)
  ...
  subjects<-DF %>%
    ...
    spread_values(
      ...
      subject_id = jnumber(subject, retired, subject_id),
      class_count = jnumber(subject, retired, classifications_count),
      retire_date = jstring(subject, retired, retired_at),

      #Next values from Species identification manifests (Cole)
      CamSD = jstring(subject, CamSD),
      CamNum = jstring(subject, CamNum),
      SDNum = jstring(subject, SD_Num),
      DTO_Img1 = jstring(subject, DateTimeOriginal_Img1),
      Event = jstring(subject, Event),
      Image1 = jstring(subject, Image1),
      Image2 = jstring(subject,Image2),
      Image3 = jstring(subject, Image3)
