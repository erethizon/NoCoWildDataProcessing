...
## Camera C041_SD013
This deployment, corresponding to deployment C0354, had images all with basically the same date time stamp, so they cannot be broken into "events". For that reason, I'm going to manually assign an event column and just assign 1 image to each event, in order of image name. I will then use that event column to make the manifest.
...
invisible(sapply(here("Code", "Functions",myFunctions), source)) #sources all of the functions in the Functions folder. #invisible function sources the functions without printing value ? and visible FALSE in the console for each function that is sourced.
...
Forcing for C041_SD013
```{r}
DF <- Exif

#Put images in order by image name; first create column based on image number, then put in order by that column
DF <- DF |>
  mutate(
    Image_number = str_sub(FileName, -12)
  )
DF <- DF |> arrange(Image_number)
...
write.csv(Manifest, here("Output", "PreZooniverse", "Manifests", "Z-15-Feb-C041_SD013-upload-manifest.csv"), row.names = F)
```
