...
cols_to_dump <-colnames(Flat[empty_cols]) #make a vector of column names for the empty columns
#now exclude Event column from cols_to_dump
cols_to_dump <- cols_to_dump[! cols_to_dump %in% 'Event']
```

So lets remove them (but keep Event which will be used later)
```{r}

Flat<-Flat %>% select(!cols_to_dump)
```
...
write.csv(correct_classifications, paste0(clean_output,"/", correctFile))
```
...
write.csv(filesToCheck, paste0(needs_checking_output,"/", needToCheckFile))
```
