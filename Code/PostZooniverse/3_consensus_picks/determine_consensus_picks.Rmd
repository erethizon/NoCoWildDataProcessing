...
```{r}
Flat<-Flat %>% select(!X)
Meta<-Flat %>% select(c(subject_ids, classification_id, user_name:array.index, workflow_id, id, round, Imj1, Imj2, Img3, CamNum, SD_card_num, Event))
Choices<-Flat %>% select(c(subject_ids, classification_id,choice, class.count, Event))
Answers<-Flat %>% select(c(subject_ids, classification_id,choice,answers.HOWMANY:answers.WHATTYPEOFHUMAN))
```
...
```{r}
subject_metrics <- Choices %>% ungroup %>%
...
```
The group of subject for which there was more than one species present in the images is that set for which the number of votes exceeds the number of classifications. We'll look at it two different ways. First, just identify those subjects and put them in a new data frame.

```{r}
MoreThan1<-cleaned_classifications %>% filter(num_votes > num_class)
```

Now let's filter out the group for which the number of classfications is equal to the number of votes AND for which there is more than one species identified.

```{r}
NeedsCheck<-cleaned_classifications %>% filter(num_votes == num_class & num_diff_species > 1)
```
...
Let's find these to compare with MoreThan1 above.

```{r}
MoreThan1a<-species_votes %>% filter(num_votes > num_class)
```
They each show the same subject_ids so things look good.

Let's filter species_votes to extract the subjects for which there is more than 1 species but the number of votes = num_class. These will be the cases in which there one species in the event but there is a discrepancy about what the species is.
...
```
Now join cleaned_classifications to Meta, Answers
```{r}
Rejoined<-left_join(cleaned_classifications, Meta)
```
