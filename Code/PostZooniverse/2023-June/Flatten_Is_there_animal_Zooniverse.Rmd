...
As of 2023, our images on the Zooniverse are put through two workflows:

- "Is there an animal?" is a workflow in which the user determines if there is a human, other animal, or nothing in the images from an event.

- "Species identification" is a workflow that takes images that were identifed as having animals and asks the users to classify them.

This file is designed to help flatten the "Is there an animal?" workflow data, which have different JSON format than the "Species Identification" workflow
...
THIS FILE IS FOR PARSING THE JSON DATA FROM THE "IS THERE AN ANIMAL" WORKFLOWS. USE THE FILE "FLATTEN_SPECIES_ID_ZOONIVERSE.RMD" TO PARSE SPECIES ID DATA.
...
```{r}
DF<-choose_file_is_there_animal_workflow()
data<-DF[[1]]
filename<-DF[[2]]
```
...
```{r}
flat_file<-flatten_is_there_animal(data) #this is slow; could we write faster code?  A problem for another day.
```
...
```{r}
output<-here("Output", "PostZooniverse", "Flattened", "Is_there_animal")
