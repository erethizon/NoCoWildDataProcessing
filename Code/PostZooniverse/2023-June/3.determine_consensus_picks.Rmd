...
```{r}
#first identify any columns that begin with "X"
Flat_cols<-colnames(Flat)
junk_cols<- Flat_cols %>% str_starts("X")
junk_cols <- str_which(junk_cols, "TRUE")

#now subset by dumping that column
Flat<-Flat %>% select(!junk_cols)

```
Now for ease of viewing, get rid of all columns that are empty. First determine which they are
```{r}
empty_cols<-sapply(Flat, function(x)all(is.na(x)))
```
The empty columns are:
task_label, id, round, batch,CamMod,CamNumber, SDNumber, Forest,For_type,round1, Img4, Img5, Img6,CamNum,SD_card_num,Event,PHase,SDCard, Lat, Long,Treatment

So lets remove them (but keep Event which will be used later)
```{r}
dump_cols<-c("task_label", "id", "round", "batch","CamMod","CamNumber", "SDNumber", "Forest","For_type","round1", "Img4", "Img5", "Img6", "CamNum","SD_card_num","Phase","SDCard", "Lat", "Long","Treatment")
Flat<-Flat %>% select(!dump_cols)
```
...
write file
```{r}
write.csv(filesToCheck, here("Output","PostZooniverse", "wkflw_16995_files_to_check-2023-06-13.csv"))
```
...
Checklist<-data.frame(subject_ids = integer(length(Subjects)),
                      ...
                      DTO = character(length(Subjects)),
                      subject_set_id = integer(length(Subjects)))
```
Now loop
```{r}
for(i in 1:length(Subjects)){
  ...
  Checklist$Forest[i]<-unique(mySample$Forest1)
  Checklist$DTO[i]<-unique(mySample$DTO)
  Checklist$subject_set_id[i]<-unique(mySample$subject_set_id)
}
```
Now save

```{r}
write.csv(Checklist, here("Output", "PostZooniverse", "check_species_wkflw16995-2023-06-13.csv"))
```
...
Assigned<-data.frame(subject_ids = integer(length(Subjects_to_keep)),
                ...
                DTO = character(length(Subjects_to_keep)),
                subject_set_id = integer(length(Subjects_to_keep)),
...
for(i in 1:length(Subjects_to_keep)){
  ...
  Assigned$DTO[i]<-unique(mySample$DTO)
  Assigned$subject_set_id[i]<-unique(mySample$subject_set_id)
...
```{r}
write.csv(Assigned, here("Output", "PostZooniverse", "AssignedSpecies_wkflow16995_2023-06-13.csv"))
```
