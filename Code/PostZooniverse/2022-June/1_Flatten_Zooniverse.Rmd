...
```{r}
subjects<-flatten_subjects_Donovan(data)
```
...
write.csv(Flattened,  paste0(output, "/",myFile,"_flat_", Sys.Date(), "_wf1_donovan.csv"))

```
## Now repeat for Kate's data:

###Step 1. Choose the classification file to work with.
 Bart_s04/Research_group/NoCoWild/Data/classification_files.
...
```{r}
subjects<-flatten_subjects_Kate(data)
```
...
write.csv(Flattened,  paste0(output, "/",myFile,"_flat_", Sys.Date(), "_wf1.1_kate.csv"))

```
## And now Cole and Maggie's data
###Step 1. Choose the classification file to work with.
 Bart_s04/Research_group/NoCoWild/Data/classification_files.
...
write.csv(Flattened,  paste0(output, "/",myFile,"_flat_", Sys.Date(), "_sp_id_cole_maggie.csv"))
