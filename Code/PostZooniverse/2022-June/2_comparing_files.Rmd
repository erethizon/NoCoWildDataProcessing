...
```{r}
DS<-read.csv(here("Output", "PostZooniverse", "Flattened", "north-country-wild-classifications.6June2022_flat_2022-06-08_wf1_donovan.csv"))

KA<-read.csv(here("Output", "PostZooniverse", "Flattened", "north-country-wild-classifications.6June2022_flat_2022-06-08_wf1.1_kate.csv"))

CWMM<-read.csv(here("Output", "PostZooniverse", "Flattened", "north-country-wild-classifications.6June2022_flat_2022-06-08_sp_id_cole_maggie.csv"))
```
...
```
Now search through each DF and get rid of the columns that only contain NAs

```{r}
DS1 <- DS[, !sapply(DS, function(x)all(is.na(x))), drop=F]
KA1 <- KA[, !sapply(KA, function(x)all(is.na(x))), drop=F]
CWMM1 <-CWMM[, !sapply(CWMM, function(x)all(is.na(x))), drop=F]
```
...
```
Now get rid of the beta test classifications from CWMM and then save it.
