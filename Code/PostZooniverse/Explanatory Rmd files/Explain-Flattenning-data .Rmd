...
myFunctions<-list.files(here("Code", "Functions", "In progress")) 
#points to files that need to be sourced

sapply(here("Code", "Functions", "In progress", myFunctions), source) #sources all of the functions in the Functions/In progress folder.
```

Now list all the functions in a folder and source them all
```{r}
my_path<-"./Functions/" #path to folder full of functions each in a .R file. the "dot" tells R to use the relative path.
source_files<-list.files(my_path, "*.R") #lists all the files in th folder
map(paste0(my_path, source_files), source) #now source all of the files
data_path<-"./Data/"
```
...
```{r}
#path to whatever data file you want - change path accordingly. 
classifications_file<-paste0(data_path, "north-country-wild-classifications.csv")
